    ...
    IBorrowerOperations
{
    ...
    function closeVessel(address _asset) external override {
        ...
        _activePoolRemoveColl(activePoolCached, _asset, msg.sender, coll);
    }
    ...
    function _moveTokensFromAdjustment(
        ...
        if (_isCollIncrease) {
            _activePoolAddColl(_asset, _activePool, _borrower, _collChange);
        } else {
            _activePoolRemoveColl(_activePool, _asset, _borrower, _collChange);
        }
    }
    ...
    function _activePoolAddColl(
        ...
        if (adminContract.getIsRewardPoolType(_asset)) {
            adminContract.dysonRewardPool(_asset).stake(_amount, _borrower);
        }
    }

    function _activePoolRemoveColl(
        IActivePool _activePool,
        address _asset,
        address _borrower,
        uint256 _amount
    ) internal nonReentrant {
        _activePool.sendAsset(_asset, _borrower, _amount);

        if (adminContract.getIsRewardPoolType(_asset)) {
            adminContract.dysonRewardPool(_asset).withdraw(_amount, _borrower);
        }
    }
