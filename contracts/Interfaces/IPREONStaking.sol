...
interface IPREONStaking {
    ...
    event StakingGainsDebtTokensWithdrawn(
        address indexed staker,
    ...
    event StakerSnapshotsUpdated(
        address _staker,
        uint256 _F_Asset,
        uint256 _F_DebtToken
    );

    // Functions --------------------------------------------------------------------------------------------------------

    function preonToken() external view returns (IERC20Upgradeable);
    ...
    function increaseFee_DebtToken(uint256 _PREONFee) external;
    ...
    function getPendingDebtTokenGain(
        address _user
    ) external view returns (uint256);
}
